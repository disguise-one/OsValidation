...
function Import-ModelConfig{
    ...
    $modifiedPath = Join-Path -Path $Global:OSValidationConfig.pathToDisguisePowerAndSecParentDir -ChildPath $PathToConfigFileFromOSValidationRoot
...
function Import-OSValidationTemplate{
    ...
    if( -not ( Test-Path $PathToTemplateFile ) ) {
        Write-Error "The OS Validation Template File [$($PathToTemplateFile)] (that should have been generated by OS Builder before calling OS Validator) cannot be found. Perhaps your TEMP directory is missing, or perhaps the OS BUilder logs will show more information"
    }

    $object = . $PathToTemplateFile

    if( -not $object ) {
        Write-Error "After importing the contents of the The OS Validation Template File [$($PathToTemplateFile)], the imported contents are blank, please check that the [$($PathToTemplateFile)] contains a valid powershell object"
    }
